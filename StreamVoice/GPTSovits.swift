...
actor GPTSovits {
    ...
    private var cachedSpeakers: [Speaker]?
    private var lastSpeakerUpdateTime: Date?
    ...
    func getSpeakerList(forceRefresh: Bool = false) async throws -> [Speaker] {
        // 如果有缓存且未过期（10分钟内），直接返回缓存
        if !forceRefresh,
           let cached = cachedSpeakers,
           let lastUpdate = lastSpeakerUpdateTime,
           Date().timeIntervalSince(lastUpdate) < 600 {
            return cached
        }
        
        let url = URL(string: "\(baseURL)/infer/spks")!
        ...
        if httpResponse.statusCode == 401 {
            throw GPTSovitsError.unauthorized
        }
        
        guard httpResponse.statusCode == 200,
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let spklist = json["spklist"] as? [String: [String]] else {
            
            // 打印详细错误信息
            if let errorString = String(data: data, encoding: .utf8) {
                print("🔴 获取角色列表失败，服务器响应：\(errorString)")
            }
            throw GPTSovitsError.invalidResponse
        }
        
        let speakers = spklist.map { Speaker(name: $0.key, emotions: $0.value) }
        
        // 更新缓存
        cachedSpeakers = speakers
        lastSpeakerUpdateTime = Date()
